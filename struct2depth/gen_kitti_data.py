...
HEIGHT = 128
STEPSIZE = 1
INPUT_DIR = 'data_generation/'
OUTPUT_DIR = 'processed_data/rgb/'
...
for d in glob.glob(INPUT_DIR + '/*/'):
    ...
    for d2 in glob.glob(d + '*/'):
        ...
        for subfolder in ['image_02/rgb']: #, 'image_03/data']:
            ct = 1
            ...
            for i in range(SEQ_LENGTH*delta, len(files)+1, delta):
                ...
                for j in range(i-SEQ_LENGTH*delta, i, delta):  # Collect frames for this sample.
                ...
                f = open(OUTPUT_DIR + '/' + imgnum + '_cam.txt', 'w')
